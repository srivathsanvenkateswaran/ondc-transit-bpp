...
test("the 2+2 seater is twelve rows of four and one rear bench of five", async () => {
  ...
  const map = await source.seatMap("AIRAVAT_CLUB_CLASS-2P2-53");
  assert.ok(map);
...
test("the aisle breaks adjacency on a 2+2 coach", () => {
  ...
  return source.seatMap("AIRAVAT_CLUB_CLASS-2P2-53").then((map) => {
    assert.ok(map);
...
test("adjacency and pairing are symmetric on every shipped map", async () => {
  for (const seatMapId of ["AIRAVAT_CLUB_CLASS-2P2-53", "PALLAKKI-2P1-30"]) {
