 ...
 */
test("a multi-seat hold's batched insert gives every seat its own row, not its neighbour's", () => {
  const store = newStore();
  const seatIds = ["U5B", "U3A", "U4A", "U3B", "U4B"];
  const hold = acquire(store, "tx1", seatIds, 1_000_000);
  assert.deepEqual(hold.seatIds, [...seatIds].sort());

  const claims = store
    .liveClaims(SERVICE, DATE)
    .map((claim) => ({ seatId: claim.seatId, holdId: claim.holdId, state: claim.state }));
  assert.deepEqual(
